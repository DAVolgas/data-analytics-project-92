...
select 
	...
	to_char(s.sale_date, 'FMDay') as day_of_week,			-- из даты продижи берем название дня недели и убираем лишние пробелы
	floor(SUM(s.quantity * p.price)) as income			-- суммарная выручка с округлением в меньшую сторону до целого числа
...
inner join products p									-- присоединяем таблицу products
	on p.product_id = s.product_id
group by e.first_name ||' '|| e.last_name, to_char(s.sale_date, 'FMDay'), extract (isodow from s.sale_date) -- группировка по необходимым полям
